 ...
 *
 * BUGS FIXED:
 * 1. Token key: reads 'access' (LoginView response key), not 'access_token'
 * 2. TimeField comparison: Django returns "09:00:00" (HH:MM:SS), JS was comparing
 *    against "09:00" → calendar always empty. Fixed with normalizeTime() helper.
 * 3. DRF validation errors (conflict salle/enseignant) were shown as [object Object].
 *    Fixed with flattenDRFErrors().
 * 4. unique_together = (groupe, jour, heure_debut): PUT must use PATCH (partial)
 *    or send all required fields. Fixed to send full payload on PUT.
 * 5. Removed setTimeout double-render hack — proper async/await used instead.
 * 6. Modal close on backdrop click added.
 * 7. FIXED: All modals now use dark theme with forced visibility using !important
 * 
 * NEW: Added view switching (Jour/Semaine/Mois) and filter functionality
 */
...
const state = {
    ...
    currentView: 'semaine', // 'jour', 'semaine', 'mois'
    currentDay:  new Date(), // For day view
    currentMonth: new Date(), // For month view
    filters: {
        langue: '',
        enseignant: '',
        salle: '',
...
// ============================================================
// API — with proper error flattening
// ============================================================
...
function getFilteredPlannings() {
    return state.plannings.filter(p => {
        ...
        if (state.filters.langue && groupe) {
            ...
            if (!langue.includes(state.filters.langue.toLowerCase())) return false;
        }
...
function renderWeekView(container) {
    ...
    weekDates.forEach(date => {
        container.appendChild(createDayColumn(date, filteredPlannings));
    });
}
...
function createDayColumn(date, plannings) {
    ...
    TIME_SLOTS.forEach(slotTime => {
        ...
        const matches = plannings.filter(p =>
            p.jour === dayNameFull &&
            normalizeTime(p.heure_debut) === slotTime
        );
...
function renderDayView(container) {
    ...
    // Single day column (wider)
    const dayDiv = document.createElement('div');
    dayDiv.className = 'day-column';
    dayDiv.style.flex = '3'; // Make it wider
    ...
    TIME_SLOTS.forEach(slotTime => {
        ...
        const matches = filteredPlannings.filter(p =>
            p.jour === dayNameFull &&
            normalizeTime(p.heure_debut) === slotTime
        );
    ...
    // Add empty columns to maintain layout (optional)
    for (let i = 0; i < 4; i++) {
...
function renderMonthView(container) {
    ...
    const headerTitle = document.querySelector('.calendar-nav h2');
    if (headerTitle) {
        headerTitle.textContent = getMonthName(state.currentMonth);
    }
    ...
    let startOffset = firstDay.getDay() - 1; // 0 = Monday
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const dayPlannings = filteredPlannings.filter(p => p.jour === dayNameFull);
        
        dayPlannings.slice(0, 3).forEach(p => { // Max 3 visible
            const groupe = state.groupes.find(g => g.id === p.groupe);
            ...
            `;
            badge.textContent = `${normalizeTime(p.heure_debut)} ${groupe?.nom_groupe || ''}`;
...
function createSessionCard(planning, groupe) {
    ...
    const hasConflict = state.plannings.some(p =>
        p.id !== planning.id &&
...
// ============================================================
// FORCED DARK THEME STYLES WITH !IMPORTANT
// ============================================================
function inp() {
...
// ============================================================
// FILTER MODAL - FORCED VISIBILITY
// ============================================================
        ...
        <div style="background:#0f172a !important;border:2px solid #6366f1 !important;border-radius:16px;padding:2rem;width:90%;max-width:450px;color:#ffffff !important;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);" onclick="event.stopPropagation()">
            ...
            <div style="display:grid;gap:1rem;">
                <div>
                    ...
                    <select id="f_langue" style="${sel()}">${langueOpts}</select>
                </div>
                <div>
                    <label style="${lbl()}">Enseignant</label>
                    <select id="f_ens" style="${sel()}">${ensOpts}</select>
                </div>
                <div>
                    <label style="${lbl()}">Salle</label>
                    <input id="f_salle" type="text" placeholder="Ex: Salle 101" value="${state.filters.salle}" style="${inp()}">
                </div>
                <div>
                    <label style="${lbl()}">Groupe</label>
                    <select id="f_groupe" style="${sel()}">${groupeOpts}</select>
                </div>
                
                <div style="display:flex;gap:0.75rem;margin-top:0.5rem;">
                    <button type="button" onclick="resetFilters()"
                            style="flex:1;padding:0.75rem;border:2px solid #475569 !important;background:#1e293b !important;color:#ffffff !important;
                                   border-radius:8px;cursor:pointer;font-weight:600;">
                        <i class="fas fa-undo"></i> Réinitialiser
                    </button>
                    <button onclick="applyFilters()"
                            style="flex:1;padding:0.75rem;background:#6366f1 !important;color:#ffffff !important;
...
function previousWeek() {
    if (state.currentView === 'jour') {
        state.currentDay.setDate(state.currentDay.getDate() - 1);
    } else if (state.currentView === 'mois') {
...
// ============================================================
// CRUD OPERATIONS - ALL WITH FORCED VISIBILITY
// ============================================================
function viewPlanning(planningId) {
    ...
    modal.innerHTML = `
        <div style="background:#0f172a !important;border:2px solid #6366f1 !important;border-radius:16px;padding:2rem;width:90%;max-width:450px;max-height:90vh;overflow-y:auto;color:#ffffff !important;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);" onclick="event.stopPropagation()">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
                <h3 style="margin:0;font-size:1.1rem;color:#ffffff !important;">
            ...
            <div style="display:grid;gap:0.75rem;margin-bottom:1.5rem;">
                ${[
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;
                                padding:0.75rem 1rem;background:#1e293b !important;border:1px solid #475569 !important;border-radius:8px;">
                        <span style="color:#94a3b8 !important;font-size:0.875rem;">${lb}</span>
                        <strong style="font-size:0.875rem;text-align:right;max-width:60%;color:#ffffff !important;">${v}</strong>
                    </div>`).join('')}
            </div>
            ...
            <div style="display:flex;gap:0.75rem;">
                ...
                <button onclick="removeModal('modal-view'); editPlanning(${planning.id});"
                        style="flex:1;padding:0.75rem;background:#3b82f6 !important;color:#ffffff !important;
                               border:none;border-radius:8px;cursor:pointer;font-weight:600;">
                    <i class="fas fa-edit"></i> Modifier
                </button>
                <button onclick="deletePlanning(${planning.id});"
                        style="flex:1;padding:0.75rem;background:#ef4444 !important;color:#ffffff !important;
                               border:none;border-radius:8px;cursor:pointer;font-weight:600;">
    ...
    modal.innerHTML = `
        <div style="background:#0f172a !important;border:2px solid #6366f1 !important;border-radius:16px;padding:2rem;width:90%;max-width:500px;max-height:90vh;overflow-y:auto;color:#ffffff !important;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);" onclick="event.stopPropagation()">
            ...
            <div style="display:grid;gap:1rem;">
                <div>
                    <label style="${lbl()}">Groupe *</label>
                    <select id="e_groupe" style="${sel()}">${groupeOpts}</select>
                </div>
                ...
                <div>
                    ...
                    <select id="e_ens" style="${sel()}">${ensOpts}</select>
                </div>
                <div id="editError" style="display:none;padding:0.75rem;background:#450a0a !important;
                     border:1px solid #f87171 !important;border-radius:8px;color:#fca5a5 !important;font-size:0.85rem;"></div>
                <div style="display:flex;gap:0.75rem;margin-top:0.5rem;">
...
async function openNewSessionModal() {
    ...
    modal.innerHTML = `
        <div style="background:#0f172a !important;border:2px solid #6366f1 !important;border-radius:16px;padding:2rem;width:90%;max-width:500px;max-height:90vh;overflow-y:auto;color:#ffffff !important;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);" onclick="event.stopPropagation()">
            ...
            <div style="display:grid;gap:1rem;">
                <div>
                    <label style="${lbl()}">Groupe *</label>
                    <select id="n_groupe" style="${sel()}">${groupeOpts}</select>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                    <div>
                        <label style="${lbl()}">Jour *</label>
                ...
                <div id="newError" style="display:none;padding:0.75rem;background:#450a0a !important;
                     border:1px solid #f87171 !important;border-radius:8px;color:#fca5a5 !important;font-size:0.85rem;"></div>
                <div style="display:flex;gap:0.75rem;margin-top:0.5rem;">
    ...
    document.getElementById('btnNewSave').addEventListener('click', async () => {
        ...
        if (!groupe) {
            errEl.textContent = 'Sélectionnez un groupe.'; errEl.style.display = 'block'; return;
        }
        if (!salle) {
            errEl.textContent = 'La salle est obligatoire.'; errEl.style.display = 'block'; return;
        }

        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Création...'; btn.disabled = true;
        ...
        if (result?.error) {
            errEl.textContent = '❌ ' + result.message;
            errEl.style.display = 'block';
            btn.innerHTML = '<i class="fas fa-plus"></i> Créer'; btn.disabled = false;
            return;
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // View toggle buttons (Jour/Semaine/Mois)
    document.querySelectorAll('.view-btn').forEach(btn => {
